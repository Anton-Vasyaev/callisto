// parent header
#include <terramagnia/graphics/opengl/gl_window_context.hpp>
// std
#include <iostream>
// 3rd party
#include <imgui.h>
...
void gl_window_context::context_key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	gl_window_context* window_context = call_functions.at(window);
	window_context->key_processing(key, scancode, action, mods);
}

void gl_window_context::context_cursor_position_callback(GLFWwindow* window, double xpos, double ypos)
{
	gl_window_context* window_context = call_functions.at(window);
	window_context->cursor_position_processing(xpos, ypos);
}

void gl_window_context::context_mouse_button_callback(GLFWwindow* window, int button, int action, int mods)
{
	gl_window_context* window_context = call_functions.at(window);
...
{
	t_inp::key_event event;
	event.key = static_cast<t_inp::key_type>(key);
...
{
	t_inp::mouse_button_event event;
	event.button = static_cast<t_inp::mouse_button>(button);
	event.action = input_types_auxiliary::get_input_action_from_glfw(action);
...
window_mode gl_window_context::gl_window_context::get_window_mode() const { return this->options.mode; }
...
void gl_window_context::start_processing() 
{
	...
	glfwSetCursorPosCallback(this->window_handler, gl_window_context::context_cursor_position_callback);
	glfwSetMouseButtonCallback(this->window_handler, gl_window_context::context_mouse_button_callback);
	...
	glfwSetFramebufferSizeCallback(this->window_handler, gl_window_context::context_framebuffer_size_callback);
...
void gl_window_context::stop_processing()
{
